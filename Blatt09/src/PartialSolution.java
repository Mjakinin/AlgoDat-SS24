...
public class PartialSolution implements Comparable<PartialSolution> {
    ...
    public PartialSolution(PartialSolution that) {
        ...
        this.moveSequence = new ArrayList<>(that.moveSequence);
        this.cost = that.cost;
    }
    ...
    public void doMove(Move move) {
        ...
        cost = moveSequence.size() + board.manhattan();
    }
    ...
    public Iterable<Move> validMoves() {
        Move lastMove = moveSequence.isEmpty()? null : moveSequence.get(moveSequence.size() - 1);
        return board.validMoves(lastMove);
    }
    ...
    public int compareTo(PartialSolution that) {
        int thisHeuristic = board.manhattan();
        int thatHeuristic = that.board.manhattan();
        int thisTotal = cost + thisHeuristic;
        int thatTotal = that.cost + thatHeuristic;
    ...
    public static void main(String[] args) {
        ...
        String filename = "C:\\Users\\maxim\\Desktop\\Algorithmen und Datenstrukturen\\HA\\MM22\\Blatt09\\src\\samples\\board-3x3-twosteps.txt";
