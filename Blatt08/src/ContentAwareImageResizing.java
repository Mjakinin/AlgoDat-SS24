...
public class ContentAwareImageResizing {
    ...
    public int[] leastContrastImageVPath() {
        ...
        int startknoten = sx * sy;
        int endknoten = sx * sy + 1;

        ShortestPathsTopological spt = new ShortestPathsTopological(graph, startknoten);
        Stack<Integer> pathStack = spt.pathTo(endknoten);

        if(pathStack == null)
        {
            throw new RuntimeException("kein Pfad");
        }
        ...
        for(int y = 0; y < sy; y++)
        {
            ...
            Coordinate coord = nodeToCoordinate(node);
            path[y] = coord.x;
        }
        return path;
    }
